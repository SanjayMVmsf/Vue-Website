          ...
          <td>
            <label>
              <input
                ...
                :checked="selectedItems.map(i => i.symbol).includes(item.symbol)"
...
import { DASBOARD_TABLE_HEADINGS, LOCALSTORAGE, DASHBOARD_TABLE_TH } from '../Commons/Constants.ts';
...
export default {
  ...
  methods: {
    valueChangeClass(value, symbol) { // eslint-disable-line
      let previousValue = '';
      let currentValue = '';
  ...
  mounted() {
    ...
    const storedSelectedItems = localStorage.getItem(LOCALSTORAGE.SELECTED_ITEMS);
